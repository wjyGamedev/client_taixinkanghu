...
public class TimeButton extends Button implements View.OnClickListener
{
	...
	{
		if (TimerMap.map == null)
			TimerMap.map = new HashMap<String, Long>();
		TimerMap.map.put(TIME, time);
		TimerMap.map.put(CTIME, System.currentTimeMillis());
		clearTimer();
	...
	{
		Log.e("yung", TimerMap.map + "");
		if (TimerMap.map == null)
			return;
		if (TimerMap.map.size() <= 0)// 这里表示没有上次未完成的计时
			return;
		long time = System.currentTimeMillis() - TimerMap.map.get(CTIME) - TimerMap.map.get(TIME);
		TimerMap.map.clear();
		if (time > 0)
			return;
		else
