 ...
 * 日历控件 功能：获得点选的日期区间
 */
public class CalendarViewXz extends View implements View.OnTouchListener
{
	...
	private int[] monthdate = new int[42]; // 日历显示数字


	private Map<Integer, Integer> m_dateStateMap = new HashMap<Integer, Integer>();
	private Map<Integer, Integer> m_dateIndexMap = new HashMap<Integer, Integer>();
	...
	private int m_todayIndex;

	//给控件设置监听事件
	private OnItemClickListener onItemClickListener;

	public CalendarViewXz(Context context)
	{
		super(context);
		init();
	}

	public CalendarViewXz(Context context, AttributeSet attrs)
	{
		super(context, attrs);
		init();
	}
	...
	{
		Log.d(TAG,
			  "[onLayout] changed:" + (changed ? "new size" : "not change") + " left:" + left + " top:" + top + " right:" + right + " " +
					  "bottom:" + bottom
			 );
		if (changed)
		{
	...
	protected void onDraw(Canvas canvas)
	{
		...
		// 按下状态，选择状态背景色
		drawDownOrSelectedBgTxkh(canvas);
		...
		if (curYearAndMonth.equals(todayYearAndMonth))
		{
			...
			m_todayIndex = curStartIndex + todayNumber - 1;
		}

		//写日期数字
		//				fillInDateNumber(canvas);
		fillInDateNumberTxkh(canvas);

		super.onDraw(canvas);
	}

	//填入数字
	private void fillInDateNumber(Canvas canvas)
	{
		for (int i = 0; i < 42; i++)
		{
			...
			if (m_todayIndex != -1 && i == m_todayIndex)
			{
	...
	//填入数字泰心康护版
	private void fillInDateNumberTxkh(Canvas canvas)
	{
		for (int i = 0; i < 42; i++)
		{
			int color = surface.textColor;
			...
			int month = monthdate[i];
			int day = date[i];
			m_dateIndexMap.put(i, year * 10000 + month * 100 + day);
		}
	}
	...
	private void calculateDate()
	{
		...
		if (monthStart > 0)
		{
			...
			{
				date[i] = dayInmonth;
				monthdate[i] = calendar.get(Calendar.MONTH);
				dayInmonth--;
			}
		...
		for (int i = 1; i < monthDay; i++)
		{
			...
			monthdate[monthStart + i] = calendar.get(Calendar.MONTH) + 1;
		}
		...
		for (int i = monthStart + monthDay; i < 42; i++)
		{
			...
			monthdate[i] = calendar.get(Calendar.MONTH) + 2;
		}
		if (curEndIndex < 42)
		{
			// 显示了下一月的
	...
	//绘制底色
	private void drawDownOrSelectedBgTxkh(Canvas canvas)
	{
		...
		if (!selectedEndDate.before(showFirstDate) && !selectedStartDate.after(showLastDate))
		{
			...
			{
				int dateIndex = -1;
				if (m_dateIndexMap.get(i) != null)
				{
					dateIndex = m_dateIndexMap.get(i);
					System.out.println("dateIndex = " + dateIndex);
					if (m_dateStateMap.get(dateIndex) == null)
					{
						System.out.println("dateIndex = " + dateIndex);
						m_dateStateMap.put(dateIndex, 0);
					}
					if (m_dateStateMap.get(dateIndex) != null)
					{
						switch (m_dateStateMap.get(dateIndex))
						{
							case 0:
							{
								surface.cellSelectedColor = getResources().getColor(R.color.main_color);
							}
							break;
							case 1:
							{
								surface.cellSelectedColor = getResources().getColor(R.color.text_color_orange);
							}
							break;
							case 2:
							{
								surface.cellSelectedColor = getResources().getColor(R.color.text_color_black);
							}
							break;
							default:
							{
	...
	private void setSelectedDateByCoorTxkh(float x, float y)
	{
	...
	private boolean isSetDateState()
	{
		if (completed == true)
		{
			if (downDate.before(selectedEndDate) || downDate.compareTo(selectedEndDate) == 0)
			{
				if (downDate.after(selectedStartDate) || downDate.compareTo(selectedStartDate) == 0)
				{
					return true;
				}
			}
	...
	public boolean onTouch(View v, MotionEvent event)
	{
		switch (event.getAction())
		{
			...
			{
				setSelectedDateByCoorTxkh(event.getX(), event.getY());
				if (downDate != null)
				{
					if (isSetDateState())
					{
						...
						int dateIndex = year * 10000 + month * 100 + day;
						if (m_dateStateMap.get(dateIndex) == null)
						{
							m_dateStateMap.put(dateIndex, 0);
						}
						if (m_dateStateMap.get(dateIndex) == 2)
						{
							m_dateStateMap.put(dateIndex, 0);
						}
						else
						{
							m_dateStateMap.put(dateIndex, m_dateStateMap.get(dateIndex) + 1);
						}
					}
					else if (isSelectMore)
					{
						if (!completed)
						{
